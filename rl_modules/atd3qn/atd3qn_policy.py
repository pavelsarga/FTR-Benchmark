...
from rl_modules.atd3qn.atd3qn_observation import ATD3QNEncoder


class ATD3QNPolicy(nn.Module):
    """
    Wraps ATD3QNEncoder's Q(S'_t, a_t) with (epsilon-greedy) discrete action
    ...
    def __init__(self, track_vel: float = 0.5, epsilon: float = 0.0, **encoder_kwargs):
        super().__init__()
        self.q_network = ATD3QNEncoder(**encoder_kwargs)
