...
@dataclass
class PanATD3QNObservation(Observation):
    ...
    def get_encoder(self) -> ObservationEncoder:
        return PanATD3QNEncoder(**(self.encoder_opts or {}))


class PanATD3QNEncoder(ObservationEncoder):
    """
    ...
    Input:  x of shape (..., HM_DIM + STATE_DIM) = (..., 15 + 4)
      - H = x[..., :15]  local terrain heights (N downsampled point-set averages)
      - E = x[..., 15:]  robot state (front/rear flipper angle, chassis pitch, ...)
